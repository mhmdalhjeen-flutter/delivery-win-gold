...
export const PUSH_APP = 'delivery';
export const SUBSCRIPTION_STATUS_KEY = 'delivery-push-subscribed-v1';
const PERMISSION_PROMPTED_KEY = 'delivery-push-permission-prompted-v1';

function isDebugEnabled() {
  try {
    return import.meta.env.DEV || localStorage.getItem('delivery-push-debug') === '1';
  } catch {
    return false;
  }
}
...
export function logPushDiagnostic(event, details = {}) {
  ...
  if (details.ok === false || details.level === 'error') {
    console.warn('[WinGold Delivery Push]', payload);
    return;
  }
  if (isDebugEnabled()) {
    console.info('[WinGold Delivery Push]', payload);
  }
}
...
export async function ensurePushSubscriptionSynced(deps = {}) {
  const getToken = deps.getToken ?? (() => localStorage.getItem('deliveryToken'));
