...
test('delivery storage key is isolated from customer/store', () => {
  assert.equal(LOGIN_RATE_LIMIT_STORAGE_KEY, 'delivery_login_rate_limit_until');
});
...
test('parseRetryAfterSeconds uses Retry-After header when present', () => {
  ...
  assert.equal(parseRetryAfterSeconds(error), 30);
});
...
test('getRemainingSeconds uses absolute expiration timestamp', () => {
  ...
  assert.equal(getRemainingSeconds(until), 2);
});

test('formatCountdownSeconds renders MM:SS', () => {
  assert.equal(formatCountdownSeconds(29), '00:29');
});
